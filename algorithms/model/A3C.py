...
class A3C_Model(nn.Module):
    def __init__(self, chev_conv_state_dim, action_dim):
        ...
        self.actor_conv = ChebConv(in_channels=chev_conv_state_dim, out_channels=5, K=3)
        self.critic_conv = ChebConv(in_channels=chev_conv_state_dim, out_channels=5, K=3)

        self.actor_vnr_1_fc = nn.Linear(1, 5)
        self.actor_vnr_2_fc = nn.Linear(1, 5)
        self.actor_vnr_3_fc = nn.Linear(1, 5)

        self.critic_vnr_1_fc = nn.Linear(1, 5)
        self.critic_vnr_2_fc = nn.Linear(1, 5)
        self.critic_vnr_3_fc = nn.Linear(1, 5)

        self.actor_fc = nn.Linear((config.SUBSTRATE_NODES + 3) * 5, action_dim)
        self.critic_fc = nn.Linear((config.SUBSTRATE_NODES + 3) * 5, 1)
    ...
    def forward(self, substrate_features, substrate_edge_index, vnr_features):
        ...
        vnr_output_1_actor = vnr_output_1_actor.view(-1, 1, 5)
        vnr_output_2_actor = vnr_output_2_actor.view(-1, 1, 5)
        vnr_output_3_actor = vnr_output_3_actor.view(-1, 1, 5)
        ...
        gcn_embedding_actor = gcn_embedding_actor.view(-1, (config.SUBSTRATE_NODES + 3) * 5)
        ...
        vnr_output_1_critic = vnr_output_1_critic.view(-1, 1, 5)
        vnr_output_2_critic = vnr_output_2_critic.view(-1, 1, 5)
        vnr_output_3_critic = vnr_output_3_critic.view(-1, 1, 5)
        ...
        gcn_embedding_critic = gcn_embedding_critic.view(-1, (config.SUBSTRATE_NODES + 3) * 5)
