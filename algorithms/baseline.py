...
class BaselineVNEAgent():
    ...
    def find_substrate_node(self, substrate_net, vnr):
        candidate_nodes = {}
        H_values = {}
        ...
        # find the candidate node in substrate_net node
        for v_node_id, v_cpu_demand in sorted_virtual_nodes:
            for s_node_id, s_cpu_capacity in substrate_net.nodes(data=True):
                if s_cpu_capacity['CPU'] >= v_cpu_demand:
                    H_values[s_node_id] = self.calculate_node_available_resource(
                        s_cpu_capacity, substrate_net[s_node_id]
                    )
            ...
            if len(H_values) == 0:
                ...
                candidate_nodes[v_node_id] = sorted(H_values.items(), key=(lambda x: x[1]), reverse=True)

            H_values = {}

        # select the appropriate substrate_net nodes
        if not postponed:
            for v_node_id, v_cpu_demand in sorted_virtual_nodes:
                unselected_nodes = []
    ...
    def find_substrate_link(self, substrate_net, vnr, embedded_nodes):
        embedded_links = {}
        ...
        # order the largest CPU in VNR
        for src, dst, bandwidth_demand in vnr.edges(data=True):
        ...
        for v_link_id, bandwidth_demand in sorted_virtual_links:
            ...
            shortest_paths[v_link_id] = nx.shortest_simple_paths(substrate_net, source=src, target=dst)
        ...
        for v_link_id, bandwidth_demand in sorted_virtual_links:
            for path in shortest_paths[v_link_id]:
                if len(path) == 2:
                    if substrate_net.edges[path]['bandwidth'] >= bandwidth_demand:
                        embedded_links[v_link_id] = (path, bandwidth_demand)
                        break
                elif len(path) > 2:
                    for node_id in range(len(path) - 1):
                        if substrate_net.edges[path[node_id], path[node_id + 1]]['bandwidth'] >= bandwidth_demand:
    ...
    def calculate_revenue_for_a_vnr(self, vnr):
        total_bandwidth = 0
        total_CPU = 0

        for v_src, v_dst, v_bandwidth in vnr.edges(data=True):
            total_bandwidth += v_bandwidth['bandwidth']
        for v_node_id, v_cpu_demand in vnr.nodes(data=True):
    ...
    def get_action(self, state):
        action = {}
        SUBSTRATE_NET = state["substrate_net"]
        VNRs_COLLECTED = state["vnrs_collected"]

        for idx, vnr in enumerate(VNRs_COLLECTED):
            embedded_nodes, postponed = self.find_substrate_node(SUBSTRATE_NET, vnr)

            if not postponed:
                embedded_links, postponed = self.find_substrate_link(self.SUBSTRATE_NET, self.VNR, embedded_nodes)

                ## Reflect the substrate_net network
